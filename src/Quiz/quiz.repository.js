...
const addQuiz = ({ question, user_id, answers }) => {
    return new Promise((resolve, reject) => {
        ...
        db.query(queryQuiz, [question, user_id], (error, quizResult) => {
            ...
            const queryAnswers = "INSERT INTO answers (option, point, quiz_id) VALUES (?, ?, ?)";
            let answerPromises = answers.map((answer) => {
                return new Promise((resolveAnswer, rejectAnswer) => {
                    db.query(queryAnswers, [answer.option, answer.point, quizId], (error) => {
            ...
            Promise.all(answerPromises)
                .then(() => resolve({ quiz_id: quizId, message: "Quiz and answers added successfully!" }))
...
const updateQuiz = (quizId, question, userId, answers) => {
    return new Promise((resolve, reject) => {
        ...
        db.beginTransaction((err) => {
            ...
            const queryUpdateQuiz = "UPDATE quizzes SET question = ?, user_id = ? WHERE quiz_id = ?";
